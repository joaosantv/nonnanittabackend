...
// --- Telegram ---
// Suas credenciais já estão sendo carregadas pelo .env
...
// --- URL do Frontend ---
// IMPORTANTE: Coloque a URL do seu site do Netlify aqui!
...
app.post('/reservas', (req, res) => {
  ...
  const mensagemWhats = `Olá ${Nome}! Confirmamos a sua reserva na Nonna Nita para o dia ${dataReserva} às ${horaReserva}. Estamos à sua espera!`;
  ...
  bot.sendMessage(String(CHAT_ID), mensagemTelegram, {
    parse_mode: 'Markdown',
    reply_markup: { inline_keyboard: [[{ text: '✅ Confirmar (Automático)', callback_data: `reserva_confirmar_${novaReserva.id}` }, { text: '❌ Recusar (Automático)', callback_data: `reserva_recusar_${novaReserva.id}` }]] }
  });
...
app.post('/pedidos', (req, res) => {
    ...
    const mensagemWhats = `Olá ${Nome}! O seu pedido na Nonna Nita foi CONFIRMADO e já pode ser retirado!`;
    const linkWhatsApp = `https://wa.me/55${telefoneLimpo}?text=${encodeURIComponent(mensagemWhats)}`;
    // -----------------------------

    const mensagemTelegram = `*Novo Pedido para Retirada!* 🛍️\n\n*Nome:* ${Nome}\n*Telefone:* ${Telefone}\n\n*Itens:*\n${novoPedido['Itens do Pedido']}\n\n*Total:* ${novoPedido['Total do Pedido']}\n\n[➡️ Avisar via WhatsApp](${linkWhatsApp})`;
    bot.sendMessage(String(CHAT_ID), mensagemTelegram, {
        parse_mode: 'Markdown',
        reply_markup: { inline_keyboard: [[{ text: '✅ Confirmar (Automático)', callback_data: `pedido_confirmar_${novoPedido.id}` }, { text: '❌ Recusar (Automático)', callback_data: `pedido_recusar_${novoPedido.id}` }]] }
    });
...
bot.on('callback_query', async (query) => {
    ...
    // Recriar o link do WhatsApp para manter na mensagem editada
    const telefoneLimpo = item.Telefone.replace(/\D/g, '');
    let linkWhatsApp = '';
    if (tipo === 'reserva') {
        const msgWhats = `Olá ${item.Nome}! A sua reserva foi ${novoStatus}.`;
        linkWhatsApp = `https://wa.me/55${telefoneLimpo}?text=${encodeURIComponent(msgWhats)}`;
        textoEditado += `\n[Continuar no WhatsApp](${linkWhatsApp})`;
    } else {
        const msgWhats = `Olá ${item.Nome}! O seu pedido foi ${novoStatus}.`;
        linkWhatsApp = `https://wa.me/55${telefoneLimpo}?text=${encodeURIComponent(msgWhats)}`;
        textoEditado += `\n[Continuar no WhatsApp](${linkWhatsApp})`;
    }
    ...
    if (item.Email) { // Apenas reservas terão e-mail
        ...
        const mensagemEmail = novoStatus === 'Confirmado'
          ? `Olá ${item.Nome}, a sua ${tipo} para o dia ${item['Data da Reserva']} às ${item['Hora da Reserva']} foi CONFIRMADA!`
          : `Olá ${item.Nome}, infelizmente a sua ${tipo} para o dia ${item['Data da Reserva']} às ${item['Hora da Reserva']} foi RECUSADA.`;
